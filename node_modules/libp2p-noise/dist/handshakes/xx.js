...
var _abstractHandshake = require("./abstract-handshake");

class XX extends _abstractHandshake.AbstractHandshake {
  initializeInitiator(prologue, s, rs, psk) {
    const name = "Noise_XX_25519_ChaChaPoly_SHA256";
    const ss = this.initializeSymmetric(name);
    this.mixHash(ss, prologue);

    const re = _buffer.Buffer.alloc(32);
  ...
  writeMessageB(hs, payload) {
    ...
    const spk = _buffer.Buffer.from(hs.s.publicKey);
  ...
  readMessageA(hs, message) {
    if ((0, _utils.isValidPublicKey)(message.ne)) {
      hs.re = message.ne;
    }
  ...
  readMessageB(hs, message) {
    ...
    if (valid1 && ns.length === 32 && (0, _utils.isValidPublicKey)(ns)) {
      hs.rs = ns;
    }
  ...
  initSession(initiator, prologue, s) {
    ...
    const rs = _buffer.Buffer.alloc(32); // no static key yet
  ...
  recvMessage(session, message) {
    let plaintext = _buffer.Buffer.alloc(0);
